...
def load_base_model_and_tokenizer(model_name='unsloth/Qwen2.5-0.5B-Instruct-bnb-4bit', max_seq_length=256):
    ...
    return unsloth.FastLanguageModel.from_pretrained(model_name=model_name,
...
def is_model_4bit_quantized(model):
    ...
    return any(isinstance(module, bitsandbytes.nn.Linear4bit) \
...
def build_sft_trainer(model, tokenizer, dataset, training_args, max_seq_length=256):
    ...
    base = {f.name: getattr(training_args, f.name)
            for f in dataclasses.fields(training_args) if f.init}
    args = trl.SFTConfig(**base,
...
def generate_reply(model, tokenizer, prompt, max_new_tokens=32):
    ...
    output_ids = model.generate(
        **inputs,
        ...
        do_sample=False # Greedy decoding (do_sample=False) always picks the argmax logit, so the same prompt yields the same reply.
    )
